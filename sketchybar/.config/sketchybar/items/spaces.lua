...
local function update_all_spaces()
    ...
    sbar.exec(cmd, function(output)
        ...
        -- Update each space individually (no batch hide to avoid flicker)
        for workspace, space in pairs(space_items) do
            ...
            local should_show = (is_visible or has_windows) and true or false

            -- Update state
            space_states[workspace] = { visible = is_visible, has_windows = has_windows }

            -- Set visibility directly (no intermediate hidden state)
            space:set({ drawing = should_show })

            if should_show then
                -- Reset icon to workspace number if no windows (will be overwritten in Step 3 if has windows)
                if not has_windows then
                    -- Get the label (workspace number) for this space
                    local ws_label = workspace <= 10 and tostring(workspace) or tostring(workspace - 10)
                    space:set({
                        icon = {
                            string = ws_label,
                            font = {
        ...
        -- Step 3: Update icons for spaces with windows (show "number icon icon" format)
        for workspace, _ in pairs(has_windows_set) do
            ...
            if ws_num and space_items[ws_num] then
                sbar.exec("aerospace list-windows --workspace " .. workspace .. " 2>/dev/null", function(windows_output)
                    ...
                    -- Show workspace number + app icons
                    local ws_label = ws_num <= 10 and tostring(ws_num) or tostring(ws_num - 10)
                    local app_icons_str = table.concat(icons, " ")
...
local function create_space_item(workspace, label, sync_index)
    ...
    local space = sbar.add("item", space_name, {
        ...
        icon = {
            ...
            padding_left  = 6,
            padding_right = 6,
            y_offset      = 0,  -- Position icon at bottom of pill
        },
    ...
    sbar.exec("command -v aerospace", function(output)
        ...
        sbar.exec("aerospace list-monitors --format '%{monitor-id} %{monitor-appkit-nsscreen-screens-id} %{monitor-is-main}' 2>/dev/null", function(monitors_output)
            ...
            local workspace_observer = sbar.add("item", "spaces_observer", {
                position    = "left",
                drawing     = false,
                updates     = "on",  -- Always receive updates even when not visible
                update_freq = 5,
            })
