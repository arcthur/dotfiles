...
bind.load({
  ...
  -- NOTE: which-key group labels (<leader>b/c/f/g/l/n/q/r/s/t/w/x) are registered
  -- natively via which-key's `opts.spec` in `lua/plugins/ui.lua` -- not as empty
  -- keymaps here (which would bind the prefix to a no-op and stall on timeoutlen).
})
